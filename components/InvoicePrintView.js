export default {
    ...
    props: ['printData', 'printA5Data', 'profile', 'getCustomerName', 'getCustomerAddress', 'getServiceName', 'getServiceUnit', 'getPrice', 'formatDate', 'formatMonthYear'],
    setup(props) {
        const calculateA5Total = (items, custId) => {
            if (!items) return 0;
            return items.reduce((acc, item) => {
                const price = item.harga_satuan !== undefined ? Number(item.harga_satuan) : (props.getPrice ? props.getPrice(custId, item.id_layanan) : 0);
                return acc + (Number(item.qty) * price);
            }, 0);
        };

        return { calculateA5Total };
    },
    template: `
        <div>
            <!-- 1. DOKUMEN INVOICE BULANAN (A4 SUPER KOMPAK) -->
                ...
                <table class="w-full text-left border-collapse border border-slate-200 text-[10px] mb-3">
                    <thead>
                        <tr class="bg-indigo-900 text-white font-semibold text-[9px]">
                            ...
                            <th class="p-1 border border-slate-200 text-center w-12">Qty</th>
                    ...
                    <tbody class="divide-y divide-slate-200">
                        ...
                        <!-- Baris Grand Total Menjadi Lebih Menonjol -->
                    ...
                    <tbody>
                        ...
                        <tr class="bg-indigo-50 font-bold">
                            ...
                            <td class="p-1.5 text-right text-indigo-900 border">
                                Rp {{ calculateA5Total(printA5Data.items, printA5Data.id_pelanggan).toLocaleString() }}
                            </td>
                        </tr>
                    </tbody>
                </table>
